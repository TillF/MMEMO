...
#base_dir="E:/till/uni/parameterisierung/Esera_2014/runs/"
sed=FALSE #consider sediments
skip_existing=FALSE #skip WASA runs that have already been done
...
{  
  ...
  #extract best parameter set to generate "paramset.txt" 
    ...
    skip=FALSE
    if (skip_existing) #check if already performed
    {
      ...
      if (file.exists(pfile))
      {
        ...
        {  
          warning(paste0("skipped run",run," because already there."))
          skip=TRUE #dont repeat this run, because it is already there
        } 
      }  
    }
  ...
  # run uncalibrated model
   if (TRUE)
    ...
    file.copy(from=paste0("init_config/."),     to="thread1", overwrite=TRUE, recursive=TRUE) 
    if (sed)
    {  
      ...
      file.copy(from=paste0("init_config_sed/", copyfiles), to=paste0("thread1/",copyfiles), overwrite=TRUE)
